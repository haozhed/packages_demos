...
static void notification_event_cb(lv_event_t *e)
{
    lv_obj_t *btn = lv_event_get_target(e);
...
void create_notification_page(lv_obj_t *parent)
{
    // 创建页面容器
    lv_obj_t *page = lv_obj_create(parent);
    lv_obj_set_size(page, LV_CIRCLE_WATCH, LV_CIRCLE_WATCH);
    lv_obj_set_flex_flow(page, LV_FLEX_FLOW_COLUMN);
    lv_obj_set_style_pad_all(page, 0, 0);
    lv_obj_set_style_border_width(page, 0, 0);
    lv_obj_set_style_bg_color(page, lv_color_hex(0xf0f4f8), 0);

    //new code
    lv_obj_set_scrollbar_mode(page, LV_SCROLLBAR_MODE_OFF);
    lv_obj_clear_flag(page, LV_OBJ_FLAG_SCROLLABLE);
    lv_obj_add_flag(page, LV_OBJ_FLAG_EVENT_BUBBLE); // 允许事件冒泡
    //new_code
    ...
    lv_label_set_text(title, "消息通知");
    lv_obj_set_style_text_font(title, font_manager_get_font(FONT_SMALL), LV_PART_MAIN);//new code
    ...
    lv_obj_align(badge, LV_ALIGN_RIGHT_MID, -15, 0);
    ...
    lv_label_set_text(badge_label, "5");
    ...
    lv_obj_set_style_bg_color(list, lv_color_hex(0xf0f4f8), 0);
    ...
    const char *contents[] = {
        "系统今晚将进行维护和更新,请提前保存您的工作",
        "约翰史密斯请求将您添加为朋友",
        "您的订单#A2356已发货,单击查看物流",
        "检测到您的帐户是从其他位置登录的,请及时检查",
        "618年中促销活动已经开始,折扣从百分之50开始",
        "请准时参加下午3点的团队会议",
        "您的帐户密码已成功修改",
        "您的年度订阅将在7天后到期,请及时续订"};
    ...
    const char *icons[] = {
        LV_SYMBOL_SETTINGS, LV_SYMBOL_USB, LV_SYMBOL_BLUETOOTH, LV_SYMBOL_WARNING,
        LV_SYMBOL_WIFI, LV_SYMBOL_BELL, LV_SYMBOL_BATTERY_FULL, LV_SYMBOL_CLOSE};

    lv_color_t colors[] = {
        lv_color_hex(0x3498db), lv_color_hex(0x2ecc71), lv_color_hex(0xe67e22), lv_color_hex(0xe74c3c),
        lv_color_hex(0x9b59b6), lv_color_hex(0x1abc9c), lv_color_hex(0x34495e), lv_color_hex(0xf1c40f)};

    // 创建消息项
    for (int i = 0; i < 8; i++)
    {
        lv_obj_t *list_btn = lv_list_add_btn(list, NULL, "");
        lv_obj_set_size(list_btn, LV_PCT(100), 80);
        lv_obj_set_style_bg_color(list_btn, lv_color_white(), 0);
        lv_obj_set_style_radius(list_btn, 12, 0);
        lv_obj_set_style_shadow_width(list_btn, 15, 0);
        lv_obj_set_style_shadow_color(list_btn, lv_color_hex(0xcccccc), 0);
        lv_obj_set_style_pad_all(list_btn, 12, 0);
        lv_obj_set_style_border_width(list_btn, 0, 0);

        // 标记前3个为未读
        if (i < 3)
        {
            lv_obj_set_style_border_side(list_btn, LV_BORDER_SIDE_LEFT, 0);
            lv_obj_set_style_border_color(list_btn, lv_color_hex(0x3498db), 0);
            lv_obj_set_style_border_width(list_btn, 4, 0);
        }

        // 添加事件处理
        lv_obj_add_event_cb(list_btn, notification_event_cb, LV_EVENT_CLICKED, NULL);

        // 创建容器布局
        lv_obj_t *cont = lv_obj_create(list_btn);
        ...
        lv_obj_set_size(icon_cont, 50, LV_PCT(100));
        ...
        lv_obj_set_style_pad_left(text_cont, 10, 0);
        ...
        lv_obj_set_style_text_font(title_label, font_manager_get_font(FONT_SMALL), 0);
        lv_obj_set_style_text_color(title_label, lv_color_hex(0x2c3e50), 0);

        // 内容
        lv_obj_t *content_label = lv_label_create(text_cont);
        lv_label_set_text(content_label, contents[i]);
        lv_obj_set_style_text_font(content_label, font_manager_get_font(FONT_SMALL), 0);
        lv_obj_set_style_text_color(content_label, lv_color_hex(0x7f8c8d), 0);
        lv_obj_set_style_pad_top(content_label, 5, 0);

        // 时间容器
        lv_obj_t *time_cont = lv_obj_create(cont);
        lv_obj_set_size(time_cont, 80, LV_PCT(100));
        ...
        if (i < 3)
        {
            lv_obj_t *unread = lv_obj_create(time_cont);
            lv_obj_set_size(unread, 8, 8);
            lv_obj_set_style_bg_color(unread, lv_color_hex(0x3498db), 0);
            lv_obj_set_style_radius(unread, 4, 0);
            lv_obj_set_style_pad_top(unread, 5, 0);
        }
    }
}
...
static void init_message_list_page_bak(lv_obj_t *msgCenterScreen)
{
    ...
    create_notification_page(msgCenterScreen);
}
