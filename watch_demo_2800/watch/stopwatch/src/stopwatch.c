...
//暂停
static void pause_stopwatch(void)
{
    if (stopwatch_running) {
...
static void exit_stopwatch(void)
{
    if (stopwatch_container) {
        // nav_go_back(PAGE_ANIM_SLIDE_RIGHT);
        lv_obj_add_flag(stopwatch_container, LV_OBJ_FLAG_HIDDEN);
        lv_timer_delete(time_timer);
        lv_timer_delete(stopwatch_timer);
        reset_stopwatch();
        
        LV_LOG_USER("Stopwatch hidden\n");
        //nav_switch_to_page(2, 1, PAGE_ANIM_SLIDE_RIGHT);
        //nav_go_back(PAGE_ANIM_SLIDE_RIGHT);
...
static lv_obj_t *create_circle_button(lv_obj_t *parent, const char *text, lv_color_t color, lv_event_cb_t event_cb)
{
    ...
    lv_obj_set_style_text_font(btn, font_manager_get_font(FONT_SMALL), 0);
...
void stopwatch_start(void) {
    ...
    // lv_obj_set_style_bg_color(stopwatch_container, lv_color_hex(0x1a1a2e), 0);

    //启用拖拽功能以支持手势识别
    // lv_obj_add_flag(stopwatch_container, LV_OBJ_FLAG_SCROLLABLE);
    // lv_obj_set_scroll_snap_x(stopwatch_container, LV_SCROLL_SNAP_NONE);
    // lv_obj_set_scroll_dir(stopwatch_container, LV_DIR_HOR);
    
    // //添加手势事件处理器
    // lv_obj_add_event_cb(stopwatch_container, gesture_event_handler, LV_EVENT_ALL, NULL);
    ...
    lv_obj_align(title_label, LV_ALIGN_CENTER, -30, -180);
    ...
    update_system_time();
    //lv_obj_set_pos(time_label, 300, 45);
    lv_obj_align(time_label, LV_ALIGN_CENTER, 30, -180);
    ...
    lv_label_set_text(stopwatch_label, "00:00.000");
    lv_obj_align(stopwatch_label, LV_ALIGN_CENTER, 0, -10);
    lv_obj_set_style_text_font(stopwatch_label, font_manager_get_font(FONT_SMALL), 0);
    ...
    time_timer = lv_timer_create(time_timer_cb, 1000 * 60, NULL);
