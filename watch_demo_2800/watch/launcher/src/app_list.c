...

{
    ...
    if (row == 2 && col == 1)//turn to timer
    {
        ...
        lv_obj_add_event_cb(item, special_icon_click_handler, LV_EVENT_CLICKED, TIMER_APP);
    }
    else if (row == 2 && col == 2) //turn to clock
    {
        ...
        lv_obj_add_event_cb(item, special_icon_click_handler, LV_EVENT_CLICKED, CLOCK_APP);

    } else if (row == 2 && col == 0) { //turn to music app
        lv_obj_set_style_bg_color(item, lv_color_hex(0x3498db), LV_PART_MAIN);
        ...
        lv_obj_add_event_cb(item, special_icon_click_handler, LV_EVENT_CLICKED, MUSIC_APP);
    } else {
...
void apps_center_start(lv_obj_t *appListCenterScreen)
{
    ...
    applist_create_list(appListCenterScreen);
}